function screwarrowR3( X_tail, X_head, varargin )
...
% L_ah = 0.075; % arrowhead length relative to overall arrow length L_ah = 0.135
% W_ah = 0.05 % width of arrowhead at base, relative to overall arrow length
% R_shaft = 0.0425; % cylinder radius relative to overall arrow length
R_shaft = 0.0225; % cylinder radius relative to overall arrow length
L_ah = 0.155; % arrowhead length relative to overall arrow length
W_ah = 0.10 % width of arrowhead at base, relative to overall arrow length
%  N = 12; % number of points for surfaces (shaft, arrowhead)
% N = 4; % for square base for infinite pitch
% AHeadColor = [11 131 222]/255;
% AHeadColor = [255,0,0]/255; %red
AHeadColor = [0,0,0]/255;
% AHeadBackColor = [7 86 145]/255;
AHeadBackColor = [178,34,34]/255;
% ShaftColor = [145,90,7]/255;
ShaftColor = [0,0,0]/255;
%ecshaftcolor
ecshaftcolor='k';
%echeadcolor
echeadcolor='k';
%arrow head transparency
AHeadalpha =1;
%default zero pitch screws
isconstraintcouple=0;
%height along z direction
scalefactorz=1;
...
while length(args) >= 2
    ...
    switch prop
        ...
        case 'AHeadalpha'
            AHeadalpha = val;
        case 'isconstraintcouple'
            isconstraintcouple = val;
        case 'scalefactorz'
            scalefactorz = val;
        otherwise
...
if isconstraintcouple ==0
 [x, y, z] = cylinder(L*R_shaft, N);
 h = hgtransform;
else 
[x, y, z] = cylinderbox(L*R_shaft*4, N);
end
%     surf(x, y, z, 'EdgeColor','k','FaceColor', ShaftColor);
%    patch(x', y', z', 'EdgeColor','None','FaceColor', ShaftColor);
% hold on
...
for row = 1:2
    ...
    Xr = Rz*Ry*[x(row,:); ...
        y(row,:); ...
        scalefactorz*L*(1-L_ah)*z(row,:)]; % stretch and rotate
...
hold on
if isconstraintcouple ==1
    fill3(xr(1,:),yr(1,:),zr(1,:), ShaftColor);
else
    ;
end
...
[dx_se, dy_se, dz_se] = sph2cart(az, el, scalefactorz*L*(1-L_ah));
shaft_endpoint = X_tail + [dx_se; dy_se; dz_se];
for row = 1:2
    Xahr = Rz*Ry*[xah(row,:); ...
        yah(row,:); ...
        scalefactorz*L*L_ah*zah(row,:)]; % stretch and rotate
...
% surface for the shaft
 surf(xr, yr, zr, 'EdgeColor',ecshaftcolor,'FaceColor', ShaftColor,'Facealpha',1,'AmbientStrength',1);
% surf(xr, yr, zr,'Parent', h, 'EdgeColor',ecshaftcolor,'FaceColor', ShaftColor,'Facealpha',1,'AmbientStrength',1);
% Make it taller
% set(h, 'Matrix', makehgtform('scale', [1 1 scalefactorz]))
% surface for the arrow head
surf(xahr, yahr, zahr, 'EdgeColor',echeadcolor,'FaceColor', AHeadColor,'Facealpha',AHeadalpha,'AmbientStrength',1)
